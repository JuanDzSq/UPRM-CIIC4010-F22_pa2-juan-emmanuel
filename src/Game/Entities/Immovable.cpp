...
Immovable::Immovable(string id, string entityName, int ox, int oy) : Entity(ox, oy, 50, 64)
{
    this->id = id;
...
void Immovable::inOverworldDraw()
{
    ...
    ofDrawBitmapString("oy:" + to_string(oy), 100 + 125 * ((stoi(id) % 10) - 1), 180);
    overworldSprite.draw(renderX, renderY, ow, oh);
}

void Immovable::fightingUpdate()
{
